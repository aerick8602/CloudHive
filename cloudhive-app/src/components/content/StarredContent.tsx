...
export function StarredContent({ accounts, uid }: any) {
  ...
  const queryKey =
    currentFolderId === "root" && !activeEmail
      ? `/api/file/all/${uid}?trashed=false&starred=true`
      : activeEmail
  ...
  return (
    <div>
      ...
      <AppBreadcrumb
        items={[
          { label: "Starred", id: "root", email: null },
      ...
      <div className="overflow-y-auto max-h-[calc(100vh-11rem)] mb-9">
        <DriveCard
          ...
          hasFolders={true}
          bgImage="Starred.svg"
          bgfirstMessage="No starred files
"
          bgsecondMessage="Add stars to things that you want to easily find later"
        />
      </div>
    </div>
  );
