...
def load_root_folders():
    ...
    if not os.path.exists(ROOT_FOLDERS_FILE):
        logger.warning("⚠️ No root folders file found. Returning an empty dictionary for now.")
        return {}

    try:
        with open(ROOT_FOLDERS_FILE, "r", encoding="utf-8") as file:
            logger.info("✅ Successfully loaded root folders file.")
...
def save_root_folders(root_folders):
    ...
    try:
        ...
        logger.info("✅ Root folder data saved successfully to the file.")
...
def get_root_folder(provider, email):
    ...
    if root_folder_id:
        ...
        logger.warning(f"⚠️ Root folder not found for {provider_key}.")
...
def set_root_folder(provider, email, folder_id):
    ...
    logger.info(f"✅ Successfully set the root folder for {provider_key} with ID: {folder_id}")
...
def remove_root_folder(provider, email):
    ...
    if provider_key in root_folders:
        ...
        save_root_folders(root_folders)
        logger.info(f"🗑️ Successfully removed the root folder for {provider_key}.")
    else:
        logger.warning(f"⚠️ No root folder found for {provider_key}. No action taken.")
